...
SKIP: {
    ...
    # Pithub::Repos->list
    {
        ...
        while ( my $row = $result->next ) {
            ok $row->{name},
                "Pithub::Repos->list: Attribute name ($row->{name})";
        }
    }
        ...
        eq_or_diff \@tags,
            [
            {
                'commit' => {
                    ...
                    'url' =>
                        'https://api.github.com/repos/plu/Pithub/commits/1c5230f42d6d3e376162591f223fc4130d671937'
                },
                'name'        => 'v0.01000',
                'tarball_url' =>
                    'https://api.github.com/repos/plu/Pithub/tarball/v0.01000',
                'zipball_url' =>
                    'https://api.github.com/repos/plu/Pithub/zipball/v0.01000'
            },
            {
                'commit' => {
                    'sha' => '09da9bff13167ca9940ff6540a7e7dcc936ca25e',
                    'url' =>
                        'https://api.github.com/repos/plu/Pithub/commits/09da9bff13167ca9940ff6540a7e7dcc936ca25e'
                },
                'name'        => 'v0.01001',
                'tarball_url' =>
                    'https://api.github.com/repos/plu/Pithub/tarball/v0.01001',
                'zipball_url' =>
                    'https://api.github.com/repos/plu/Pithub/zipball/v0.01001'
            }
            ],
    ...
    {
        my $result = $p->repos->commits->compare(
            user => 'plu',      repo => 'Pithub',
            base => 'v0.01008', head => 'master'
        );
        is $result->success, 1, 'Pithub::Repos::Commits->compare successful';
        is $result->content->{status}, 'ahead',
            'Pithub::Repos::Commits->compare status';
        ok $result->content->{total_commits} >= 2,
            'Pithub::Repos::Commits->compare total_commits';
    }

    # Pithub::Repos::Commits->get
    {
        my $result = $p->repos->commits->get(
            user => 'plu', repo => 'Pithub',
        ...
        eq_or_diff $result->content->{commit},
            ...
            'tree'    => {
                ...
                'url' =>
                    'https://api.github.com/repos/plu/Pithub/git/trees/7e6152aa778dd2b92c0f41c4ba243ad7482d46ab'
            },
            'url' =>
                'https://api.github.com/repos/plu/Pithub/git/commits/7e351527f62acaaeadc69acf2b80c38e48214df8'
            },
    ...
    {
        my $result
            = $p->repos->watching->list( user => 'plu', repo => 'Pithub' );
        is $result->success, 1, 'Pithub::Repos::Watching->list successful';
        like $result->content->[0]{id}, qr{^\d+$},
            'Pithub::Repos::Watching->list: Attribute id';
    }
}
...
SKIP: {
    ...
    # Pithub::Repos->create
    {
        ...
        ok grep( $_ eq $result->code, qw(201 422) ),
            'Pithub::Repos->create HTTP status';
    }

    {

        # Pithub::Repos::Watching->is_watching
        ok $p->repos->watching->is_watching->success,
            'Pithub::Repos::Watching->is_watching successful';

        # Pithub::Repos::Watching->is_watching
        ok !$p->repos->watching->is_watching(
            user  => 'plu',
            repos => 'Pithub'
            )->success,
            'Pithub::Repos::Watching->is_watching not successful';
    }

    {

        # Pithub::Repos::Watching->list_repos
        is $p->repos->watching->list_repos->first->{name}, $repo,
            'Pithub::Repos::Watching->list_repos name';

        # Pithub::Repos::Watching->start_watching
        ok $p->repos->watching->start_watching(
            user => 'plu',
            repo => 'Pithub'
            )->success,
            'Pithub::Repos::Watching->start_watching successful';

        # Pithub::Repos::Watching->list_repos
        is $p->repos->watching->list_repos->first->{name}, 'Pithub',
            'Pithub::Repos::Watching->list_repos new watched repo';

        # Pithub::Repos::Watching->stop_watching
        ok $p->repos->watching->stop_watching(
            user => 'plu',
            repo => 'Pithub'
            )->success,
            'Pithub::Repos::Watching->stop_watching successful';

        # Pithub::Repos::Watching->list_repos
        isnt $p->repos->watching->list_repos->content->[-1]->{name}, 'Pithub',
            'Pithub::Repos::Watching->list_repos not watching anymore';
    }

    {
        ...
        is $p->repos->downloads->get( download_id => $id )
            ->content->{description}, 'This test (t/live.t)',
            'Pithub::Repos::Downloads->get new download';

        # Pithub::Repos::Downloads->list
        is $p->repos->downloads->list->first->{description},
            'This test (t/live.t)',
            'Pithub::Repos::Downloads->list new download';

        # Pithub::Repos::Downloads->delete
        ok $p->repos->downloads->delete( download_id => $id )->success,
            'Pithub::Repos::Downloads->delete successful';

        # Pithub::Repos::Downloads->get
        ok !$p->repos->downloads->get( download_id => $id )->success,
            'Pithub::Repos::Downloads->get not successful after delete';
    }

    {
        ...
        my $key_id = $p->repos->keys->create(
            data => {
                ...
                key   =>
                    'ssh-rsa ',
            }
        )->content->{id};

        # Pithub::Repos::Keys->get
        is $p->repos->keys->get( key_id => $key_id )->content->{title},
            'someone@somewhere', 'Pithub::Repos::Keys->get title attribute';

        # Pithub::Repos::Keys->list
        is $p->repos->keys->list->first->{title}, 'someone@somewhere',
            'Pithub::Repos::Keys->list title attribute';

        # Pithub::Repos::Keys->delete
        ok $p->repos->keys->delete( key_id => $key_id )->success,
            'Pithub::Repos::Keys->delete successful';

        # Pithub::Repos::Keys->get
        ok !$p->repos->keys->get( key_id => $key_id )->success,
            'Pithub::Repos::Keys->get not successful after delete';
    }

    {
        ...
        ok $p->repos->update(
            ...
            'Pithub::Repos->update successful';

        # Pithub::Repos->get
        is $p->repos->get( repo => $repo )->content->{description}, "foo $$",
            'Pithub::Repos->get description after update';
    }

    {

        # Pithub::Repos::Collaborators->is_collaborator
        ok !$p->repos->collaborators->is_collaborator( collaborator => 'plu' )
            ->success,
            'Pithub::Repos->Collaborators->is_collaborator not successful yet';

        # Pithub::Repos::Collaborators->add
        ok $p->repos->collaborators->add( collaborator => 'plu' )->success,
            'Pithub::Repos->Collaborators->add successful';

        # Pithub::Repos::Collaborators->is_collaborator
        ok $p->repos->collaborators->is_collaborator( collaborator => 'plu' )
            ->success,
            'Pithub::Repos->Collaborators->is_collaborator successful now';

        # Pithub::Repos::Collaborators->list
        is $p->repos->collaborators->list->content->[-1]->{login}, 'plu',
            'Pithub::Repos->Collaborators->list first attribute login';

        # Pithub::Repos::Collaborators->remove
        ok $p->repos->collaborators->remove( collaborator => 'plu' )->success,
            'Pithub::Repos->Collaborators->remove successful now';
...

    {
        ...
        like $comment_id, qr{^\d+$},
            'Pithub::Repos::Commits->create_comment returned comment id';

        # Pithub::Repos::Commits->list_comments
        is $p->repos->commits->list_comments->content->[-1]->{id},
            $comment_id,
            'Pithub::Repos::Commits->list_comments last comment id';
        ...
        is $p->repos->commits->get_comment( comment_id => $comment_id )
            ->content->{body}, 'updated comment',
            'Pithub::Repos::Commits->get_comment after update';

        # Pithub::Repos::Commits->delete_comment
        ok $p->repos->commits->delete_comment( comment_id => $comment_id )
            ->success, 'Pithub::Repos::Commits->delete_comment successful';

        # Pithub::Repos::Commits->get_comment
        ok !$p->repos->commits->get_comment( comment_id => $comment_id )
            ->success,
            'Pithub::Repos::Commits->get_comment not successful after delete';
    }

    {
        ...
        ok grep( $_ eq $result->code, qw(201 422) ),
            'Pithub::Repos::Forks->create HTTP status';

        # Pithub::Repos::Forks->list
        is $p->repos->forks->list( user => $org, repo => $org_repo )
            ->first->{name}, 'buhtip-org-repo',
            'Pithub::Repos::Forks->list after create';
    }

    {
        ...
        like $hook_id, qr{^\d+$},
            'Pithub::Repos::Hooks->create returned hook id';

        # Pithub::Repos::Hooks->list
        is $p->repos->hooks->list->content->[-1]->{id}, $hook_id,
            'Pithub::Repos::Hooks->list last hook id';
        ...
        is $p->repos->hooks->get( hook_id => $hook_id )
            ->content->{config}{room}, 'pithub',
            'Pithub::Repos::Hooks->get after update';

        # Pithub::Repos::Hooks->test
        is $p->repos->hooks->test( hook_id => $hook_id )->code, 204,
            'Pithub::Repos::Hooks->test successful';

        # Pithub::Repos::Hooks->delete
        ok $p->repos->hooks->delete( hook_id => $hook_id )->success,
            'Pithub::Repos::Hooks->delete successful';

        # Pithub::Repos::Hooks->get
        ok !$p->repos->hooks->get( hook_id => $hook_id )->success,
